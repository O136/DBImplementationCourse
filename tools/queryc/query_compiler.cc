...
namespace imlab {
    void QueryCompiler::Compile(std::string &in, std::string &path) {
        ...
        auto algebraTreeRoot = BuildAlgebraTree(selectStatement);
        ...
        stream << "gcc -O3 -fPIC -rdynamic -I ../include -shared -o "
               << COMPILED_QUERY_LIB
               << " "
               << path;
    ...
    std::unique_ptr<Print> QueryCompiler::BuildAlgebraTree(SelectStatement &selectStatement) {
